...
abstract class element {
    ...
    /**
     * @var string The Bold Font constant.
     */
    const FD_BOLD = 'B';

    /**
     * @var string The Italic Font constant.
     */
    const FD_ITALIC = 'I';

    /**
     * @var string The Underline Font constant.
     */
    const FD_UNDERLINE = 'U';

    /**
     * @var string The Line-through Font constant.
     */
    const FD_LINETHROUGH = 'D';

    /**
     * @var string The Overline Font constant.
     */
    const FD_OVERLINE = 'O';
    ...
    /**
     * @var string The font decoration.
     */
    ...
    /**
     * Returns the font decoration.
     *
     * @return string
     */
    public function get_fontdecoration() {
    ...
    public function definition_after_data($mform) {
        ...
        $properties = [
            ...
            'fontdecoration' => str_split($this->fontdecoration, 1),
            'colour' => $this->colour,
            'posx' => $this->posx,
            'posy' => $this->posy,
            'width' => $this->width,
    ...
    public function save_form_elements($data) {
        ...
        if (!empty($this->id)) { // Must be updating a record in the database.
            $element->id = $this->id;
            $return = $DB->update_record('block_coupon_elements', $element);

            \block_coupon\event\element_updated::create_from_element($this)->trigger();

            return $return;
        } else { // Must be adding a new one.
            $element->element = $data->element;
            $element->pageid = $data->pageid;
            $element->sequence = \block_coupon\template\element_helper::get_element_sequence($element->pageid);
            $element->timecreated = time();
            $element->id = $DB->insert_record('block_coupon_elements', $element, true);
            $this->id = $element->id;

            \block_coupon\event\element_created::create_from_element($this)->trigger();
     ...
     *
     * @param \block_coupon\edit_element_form $editelementform
     */
    public function set_edit_element_form($editelementform) {
     ...
     *
     * @return \block_coupon\edit_element_form
     */
